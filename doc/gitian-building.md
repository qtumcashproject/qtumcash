Gitian building
================
*Setup instructions for a Gitian build of QtumCash Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the QtumCash
...
- [Building QtumCash Core](#building-qtumcash-core)
...
Clone the git repositories for qtumcash and Gitian.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/qtumcashproject/qtumcash --recursive
git clone https://github.com/qtumcashproject/gitian.sigs.git
```
...
```bash
sudo cp /home/debian/qtumcash/contrib/gitian-descriptors/xenial /usr/share/debootstrap/scripts
```
...
in the qtumcash repository under 'Fetch and create inputs' to install sources which require
...
To build QtumCash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the qtumcash repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/qtumcash/.git/
...
For example:
```bash
URL=https://github.com/laanwj/qtumcash.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit qtumcash=${COMMIT} --url qtumcash=${URL} ../qtumcash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit qtumcash=${COMMIT} --url qtumcash=${URL} ../qtumcash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit qtumcash=${COMMIT} --url qtumcash=${URL} ../qtumcash/contrib/gitian-descriptors/gitian-osx.yml
# if wanting to use a different version of eth-cpp-qtumcash:
./bin/gbuild --commit qtumcash=${COMMIT},cpp-eth-qtumcash=${ETHCOMMIT} --url qtumcash=${URL},cpp-eth-qtumcash=${ETHURL} ../qtumcash/contrib/gitian-descriptors/gitian-linux.yml
```
...
and the qtumcash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../qtumcash/contrib/gitian-descriptors/*|sort|uniq )
...
cd /some/root/path/
git clone https://github.com/qtumcashproject/qtumcash-detached-sigs.git

BTCPATH=/some/root/path/qtumcash
SIGPATH=/some/root/path/qtumcash-detached-sigs

./bin/gbuild --url qtumcash=${BTCPATH},signature=${SIGPATH} ../qtumcash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/qtumcash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/qtumcash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/qtumcash-osx-build.assert
```
...
[qtumcashproject/gitian.sigs](https://github.com/qtumcashproject/gitian.sigs/) repository, or if that's not possible create a pull
request. You can also mail the files to Jordan Earls (earlz@qtumcash.org) and he will commit them.
