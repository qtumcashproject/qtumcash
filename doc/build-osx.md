...
1. Clone the QtumCash Core source code:
    ```shell
...
2.  Build QtumCash Core:
    ...
    Configure and build the headless QtumCash Core binaries as well as the GUI (if Qt is found).
...
## Running
QtumCash Core is now available at `./src/qtumcashd`

Before running, you may create an empty configuration file:
```shell
mkdir -p "/Users/${USER}/Library/Application Support/QtumCash"

touch "/Users/${USER}/Library/Application Support/QtumCash/qtumcash.conf"

chmod 600 "/Users/${USER}/Library/Application Support/QtumCash/qtumcash.conf"
```

The first time you run qtumcashd, it will start downloading the blockchain. This process could
take many hours, or even days on slower than average systems.

You can monitor the download process by looking at the debug.log file:
```shell
tail -f $HOME/Library/Application\ Support/QtumCash/debug.log
```

## Other commands:
```shell
./src/qtumcashd -daemon      # Starts the qtumcash daemon.
./src/qtumcash-cli --help    # Outputs a list of command-line options.
./src/qtumcash-cli help      # Outputs a list of RPC commands when the daemon is running.
```
