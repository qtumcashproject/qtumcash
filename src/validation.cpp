...
#if defined(NDEBUG)
# error "QtumCash cannot be compiled without assertions."
#endif
...
std::unique_ptr<QtumCashState> globalState;
...
bool fAddressIndex = false; // qtumcash
#endif
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if(tx.HasCreateOrCall()){
        ...
        QtumCashDGP qtumcashDGP(globalState.get(), fGettingValuesDGP);
        uint64_t minGasPrice = qtumcashDGP.getMinGasPrice(::ChainActive().Tip()->nHeight + 1);
        uint64_t blockGasLimit = qtumcashDGP.getBlockGasLimit(::ChainActive().Tip()->nHeight + 1);
        size_t count = 0;
        ...
        QtumCashTxConverter converter(tx, NULL, NULL, contractflags);
        ExtractQtumCashTX resultConverter;
        if(!converter.extractionQtumCashTransactions(resultConverter)){
        ...
        std::vector<QtumCashTransaction> qtumcashTransactions = resultConverter.first;
        std::vector<EthTransactionParams> qtumcashETP = resultConverter.second;

        dev::u256 sumGas = dev::u256(0);
        dev::u256 gasAllTxs = dev::u256(0);
        for(QtumCashTransaction qtumcashTransaction : qtumcashTransactions){
            sumGas += qtumcashTransaction.gas() * qtumcashTransaction.gasPrice();
            ...
            if(txMinGasPrice != 0) {
                txMinGasPrice = std::min(txMinGasPrice, qtumcashTransaction.gasPrice());
            } else {
                txMinGasPrice = qtumcashTransaction.gasPrice();
            }
            VersionVM v = qtumcashTransaction.getVersion();
            if(v.format!=0)
            ...
            if(qtumcashTransaction.gas() < gArgs.GetArg("-minmempoolgaslimit", MEMPOOL_MIN_GAS_LIMIT))
            ...
            if(qtumcashTransaction.gas() < MINIMUM_GAS_LIMIT && v.rootVM != 0)
            ...
            if(qtumcashTransaction.gas() > UINT32_MAX)
            ...
            gasAllTxs += qtumcashTransaction.gas();
            ...
            if(v.rootVM!=0 && (uint64_t)qtumcashTransaction.gasPrice() < minGasPrice)
...
bool MemPoolAccept::Finalize(ATMPArgs& args, Workspace& ws)
{
    ...
    for (CTxMemPool::txiter it : allConflicting)
    {
        LogPrint(BCLog::MEMPOOL, "replacing tx %s with %s for %s QTH additional fees, %d delta bytes\n",
...
DisconnectResult CChainState::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    globalState->setRoot(uintToh256(pindex->pprev->hashStateRoot)); // qtumcash
    globalState->setRootUTXO(uintToh256(pindex->pprev->hashUTXORoot)); // qtumcash
...
std::vector<ResultExecute> CallContract(const dev::Address& addrContract, std::vector<unsigned char> opcode, const dev::Address& sender, uint64_t gasLimit){
    ...
    QtumCashTransaction callTransaction(0, 1, dev::u256(gasLimit), addrContract, opcode, dev::u256(0));
    callTransaction.forceSender(senderAddress);
    callTransaction.setVersion(VersionVM::GetEVMDefault());

    
    ByteCodeExec exec(block, std::vector<QtumCashTransaction>(1, callTransaction), blockGasLimit, pblockindex);
...
void writeVMlog(const std::vector<ResultExecute>& res, const CTransaction& tx, const CBlock& block){
    boost::filesystem::path qtumcashDir = GetDataDir() / "vmExecLogs.json";
    std::stringstream ss;
    if(fIsVMlogFile){
        ss << ",";
    } else {
        std::ofstream file(qtumcashDir.string(), std::ios::out | std::ios::app);
        file << "{\"logs\":[]}";
        file.close();
    }
    ...
    std::ofstream file(qtumcashDir.string(), std::ios::in | std::ios::out);
...
bool ByteCodeExec::performByteCode(dev::eth::Permanence type){
    for(QtumCashTransaction& tx : txs){
        //validate VM version
        ...
        if(!tx.isCreation() && !globalState->addressInUse(tx.receiveAddress())){
            ...
            result.push_back(ResultExecute{execRes, QtumCashTransactionReceipt(dev::h256(), dev::h256(), dev::u256(), dev::eth::LogEntries()), CTransaction()});
            continue;
        }
...
bool QtumCashTxConverter::extractionQtumCashTransactions(ExtractQtumCashTX& qtumcashtx){
    ...
    std::vector<QtumCashTransaction> resultTX;
    ...
    qtumcashtx = std::make_pair(resultTX, resultETP);
    return true;
}

bool QtumCashTxConverter::receiveStack(const CScript& scriptPubKey){
    sender = false;
...
bool QtumCashTxConverter::parseEthTXParams(EthTransactionParams& params){
    try{
...
QtumCashTransaction QtumCashTxConverter::createEthTX(const EthTransactionParams& etp, uint32_t nOut){
    QtumCashTransaction txEth;
    if (etp.receiveAddress == dev::Address() && opcode != OP_CALL){
        txEth = QtumCashTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.code, dev::u256(0));
    }
    else{
        txEth = QtumCashTransaction(txBit.vout[nOut].nValue, etp.gasPrice, etp.gasLimit, etp.receiveAddress, etp.code, dev::u256(0));
    }
...
size_t QtumCashTxConverter::correctedStackSize(size_t size){
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck)
{
    ...
    ///////////////////////////////////////////////// // qtumcash
    QtumCashDGP qtumcashDGP(globalState.get(), fGettingValuesDGP);
    globalSealEngine->setQtumCashSchedule(qtumcashDGP.getGasSchedule(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1) ));
    uint32_t sizeBlockDGP = qtumcashDGP.getBlockSize(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t minGasPrice = qtumcashDGP.getMinGasPrice(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    uint64_t blockGasLimit = qtumcashDGP.getBlockGasLimit(pindex->nHeight + (pindex->nHeight+1 >= chainparams.GetConsensus().QIP7Height ? 0 : 1));
    ...
    if (block.vtx.empty() || block.vtx.size() > dgpMaxBlockSize || ::GetSerializeSize(block, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS) > dgpMaxBlockSize) // qtumcash
...
///////////////////////////////////////////////////////////////////////////////////////// qtumcash
        ...
        if(tx.HasCreateOrCall() && !hasOpSpend){
            ...
            QtumCashTxConverter convert(tx, &view, &block.vtx, contractflags);

            ExtractQtumCashTX resultConvertQtumCashTX;
            if(!convert.extractionQtumCashTransactions(resultConvertQtumCashTX)){
            ...
            if(!CheckMinGasPrice(resultConvertQtumCashTX.second, minGasPrice))
            ...
            dev::u256 gasAllTxs = dev::u256(0);
            ByteCodeExec exec(block, resultConvertQtumCashTX.first, blockGasLimit, pindex->pprev);
            ...
            for(QtumCashTransaction& qtx : resultConvertQtumCashTX.first){
            ...
            {
                for(size_t k = 0; k < resultConvertQtumCashTX.first.size(); k ++){
                    ...
                    tri.push_back(TransactionReceiptInfo{
                        ...
                        tx.GetHash(),
                        uint32_t(i),
                        resultConvertQtumCashTX.first[k].from(),
                        resultConvertQtumCashTX.first[k].to(),
                        ...
                        resultConvertQtumCashTX.first[k].getNVout()
                    });
                }
...
bool CChainState::ConnectTip(CValidationState& state, const CChainParams& chainparams, CBlockIndex* pindexNew, const std::shared_ptr<const CBlock>& pblock, ConnectTrace& connectTrace, DisconnectedBlockTransactions &disconnectpool)
{
    ...
    LogPrint(BCLog::BENCH, "  - Load block from disk: %.2fms [%.2fs]\n", (nTime2 - nTime1) * MILLI, nTimeReadFromDisk * MICRO);
    {
        ...
        dev::h256 oldHashStateRoot(globalState->rootHash()); // qtumcash
        dev::h256 oldHashUTXORoot(globalState->rootHashUTXO()); // qtumcash
        ...
        if (!rv) {
            ...
            globalState->setRoot(oldHashStateRoot); // qtumcash
            globalState->setRootUTXO(oldHashUTXORoot); // qtumcash
...
//////////////////////////////////////////////////////////////////////////
    ...
    for (pindex = ::ChainActive().Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        ///////////////////////////////////////////////////////////////////// // qtumcash
        uint32_t sizeBlockDGP = qtumcashDGP.getBlockSize(pindex->nHeight);
        dgpMaxBlockSize = sizeBlockDGP ? sizeBlockDGP : dgpMaxBlockSize;
        updateBlockSizeParams(dgpMaxBlockSize);
        /////////////////////////////////////////////////////////////////////
    ...
    try {
        ...
        while (!blkdat.eof()) {
            ...
            try {
                ...
                // But for QtumCash PoS we need to sync this after every block to ensure txdb is populated for
