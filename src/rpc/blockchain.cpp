...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    result.pushKV("hashStateRoot", blockindex->hashStateRoot.GetHex()); // qtumcash
    result.pushKV("hashUTXORoot", blockindex->hashUTXORoot.GetHex()); // qtumcash
...
UniValue blockToJSON(const CBlock& block, const CBlockIndex* tip, const CBlockIndex* blockindex, bool txDetails)
{
    ...
    result.pushKV("hashStateRoot", block.hashStateRoot.GetHex()); // qtumcash
    result.pushKV("hashUTXORoot", block.hashUTXORoot.GetHex()); // qtumcash
...
UniValue transactionReceiptToJSON(const QtumCashTransactionReceipt& txRec)
{
    UniValue result(UniValue::VOBJ);
...
UniValue callcontract(const JSONRPCRequest& request)
{
    ...
    if(request.params.size() >= 3){
        CTxDestination qtumcashSenderAddress = DecodeDestination(request.params[2].get_str());
        if (IsValidDestination(qtumcashSenderAddress)) {
            const PKHash *keyid = boost::get<PKHash>(&qtumcashSenderAddress);
...
UniValue gettxout(const JSONRPCRequest& request)
{
            ...
            "     \"addresses\" : [          (array of string) array of qtumcash addresses\n"
