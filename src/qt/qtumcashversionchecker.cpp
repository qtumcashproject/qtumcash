...
#define paternVersion "qtumcash-([0-9]+\\.)?([0-9]+\\.)?([0-9]+)-"

QtumCashVersionChecker::QtumCashVersionChecker(QObject *parent) : QObject(parent)
{
    currentVersion = Version(CLIENT_VERSION_MAJOR, CLIENT_VERSION_MINOR, CLIENT_VERSION_REVISION);
}

QtumCashVersionChecker::~QtumCashVersionChecker()
{

}

bool QtumCashVersionChecker::newVersionAvailable()
{
    Version maxReleaseVersion = getMaxReleaseVersion();
    return maxReleaseVersion > currentVersion;
}

QList<Version> QtumCashVersionChecker::getVersions()
{
    QNetworkAccessManager manager;
    QNetworkReply *response = manager.get(QNetworkRequest(QUrl(QTH_RELEASES)));
    QEventLoop event;
...
Version QtumCashVersionChecker::getMaxReleaseVersion()
{
