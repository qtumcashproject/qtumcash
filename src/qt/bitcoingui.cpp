...
#include <qt/qtumcashversionchecker.h>
    ...
    m_network_style(networkStyle)
{
    ...
    qtumcashVersionChecker = new QtumCashVersionChecker(this);
#ifdef ENABLE_WALLET
    if(enableWallet) {
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a QtumCash address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and qtumcash: URIs)"));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your QtumCash addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified QtumCash addresses"));
    ...
    openAction->setStatusTip(tr("Open a qtumcash: URI or payment request"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible QtumCash command-line options").arg(PACKAGE_NAME));
...
void BitcoinGUI::setClientModel(ClientModel *_clientModel)
{
    ...
    {
        // Check for updates
        if(_clientModel->getOptionsModel()->getCheckForUpdates() && qtumcashVersionChecker->newVersionAvailable())
        {
            QString link = QString("<a href=%1>%2</a>").arg(QTH_RELEASES, QTH_RELEASES);
            QString message(tr("New version of QtumCash wallet is available on the QtumCash source code repository: <br /> %1. <br />It is recommended to download it and update this application").arg(link));
...
void BitcoinGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        tooltip = tr("%n active connection(s) to QtumCash network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
