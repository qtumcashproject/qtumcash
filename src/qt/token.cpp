...
bool ToHash160(const std::string& strQtumCashAddress, std::string& strHash160)
{
    CTxDestination qtumcashAddress = DecodeDestination(strQtumCashAddress);
    if(!IsValidDestination(qtumcashAddress))
        return false;
    const PKHash * keyid = boost::get<PKHash>(&qtumcashAddress);
    if(keyid){
...
bool ToQtumCashAddress(const std::string& strHash160, std::string& strQtumCashAddress)
{
    uint160 key(ParseHex(strHash160.c_str()));
    PKHash keyid(key);
    CTxDestination qtumcashAddress = keyid;
    if(IsValidDestination(qtumcashAddress)){
        strQtumCashAddress = EncodeDestination(qtumcashAddress);
        return true;
    }
    return false;
}
...
bool Token::execEvents(int64_t fromBlock, int64_t toBlock, int func, std::vector<TokenEvent> &tokenEvents)
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        for(int i = 0; i < listLog.size(); i++)
        {
            ...
            ToQtumCashAddress(tokenEvent.sender, tokenEvent.sender);
            tokenEvent.receiver = topicsList[2].toString().toStdString().substr(24);
            ToQtumCashAddress(tokenEvent.receiver, tokenEvent.receiver);
