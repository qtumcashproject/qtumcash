 ...
 */
static const char* BITCOIN_PID_FILENAME = "qtumcashd.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    util::ThreadRename("qtumcash-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/qtumcashproject/qtumcash>";
    const std::string URL_WEBSITE = "<https://qtumcash.org>";
...
void DeleteBlockChainData()
{
    ...
    fs::remove_all(GetDataDir() / "stateQtumCash");
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                fs::path qtumcashStateDir = GetDataDir() / "stateQtumCash";
                bool fStatus = fs::exists(qtumcashStateDir);
                const std::string dirQtumCash(qtumcashStateDir.string());
                const dev::h256 hashDB(dev::sha3(dev::rlp("")));
                dev::eth::BaseState existsQtumCashstate = fStatus ? dev::eth::BaseState::PreExisting : dev::eth::BaseState::Empty;
                globalState = std::unique_ptr<QtumCashState>(new QtumCashState(dev::u256(0), QtumCashState::openDB(dirQtumCash, hashDB, dev::WithExisting::Trust), dirQtumCash, existsQtumCashstate));
                dev::eth::ChainParams cp((chainparams.EVMGenesisInfo(dev::eth::Network::qtumcashMainNetwork)));
                globalSealEngine = std::unique_ptr<dev::eth::SealEngineFace>(cp.createSealEngine());

                pstorageresult.reset(new StorageResults(qtumcashStateDir.string()));
                if (fReset) {
