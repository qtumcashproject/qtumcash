...
const char * const BITCOIN_CONF_FILENAME = "qtumcash.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\QtumCash
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\QtumCash
    // Mac: ~/Library/Application Support/QtumCash
    // Unix: ~/.qtumcash
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QtumCash";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/QtumCash";
#else
    // Unix
    return pathRet / ".qtumcash";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("QtumCash Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The QtumCash Core Developers";
    }
