...
std::unique_ptr<CBlockTemplate> BlockAssembler::CreateNewBlock(const CScript& scriptPubKeyIn, bool fMineWitnessTx, bool fProofOfStake, int64_t* pTotalFees, int32_t txProofTime, int32_t nTimeLimit)
{
    ...
    globalSealEngine->setQtumCashSchedule(qtumcashDGP.getGasSchedule(nHeight));
    uint32_t blockSizeDGP = qtumcashDGP.getBlockSize(nHeight);
    minGasPrice = qtumcashDGP.getMinGasPrice(nHeight);
    ...
    hardBlockGasLimit = qtumcashDGP.getBlockGasLimit(nHeight);
...
bool BlockAssembler::AttemptToAddContractToBlock(CTxMemPool::txiter iter, uint64_t minGasPrice) {
    ...
    QtumCashTxConverter convert(iter->GetTx(), NULL, &pblock->vtx, contractflags);

    ExtractQtumCashTX resultConverter;
    if(!convert.extractionQtumCashTransactions(resultConverter)){
    ...
    for(QtumCashTransaction qtumcashTransaction : qtumcashTransactions){
        txGas += qtumcashTransaction.gas();
        if(txGas > txGasLimit) {
            // Limit the tx gas limit by the soft limit if such a limit has been specified.
            return false;
        }

        if(bceResult.usedGas + qtumcashTransaction.gas() > softBlockGasLimit){
            //if this transaction's gasLimit could cause block gas limit to be exceeded, then don't add it
            return false;
        }
        if(qtumcashTransaction.gasPrice() < minGasPrice){
    ...
    ByteCodeExec exec(*pblock, qtumcashTransactions, hardBlockGasLimit, ::ChainActive().Tip());
...
void ThreadStakeMiner(CWallet *pwallet, CConnman* connman)
{
    ...
    std::string threadName = "qtumcashstake";
...
void StakeQtumCashs(bool fStake, CWallet *pwallet, CConnman* connman, boost::thread_group*& stakeThread)
{
