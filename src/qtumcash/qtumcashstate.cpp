...
#include <qtumcash/qtumcashstate.h>

using namespace std;
using namespace dev;
using namespace dev::eth;

QtumCashState::QtumCashState(u256 const& _accountStartNonce, OverlayDB const& _db, const string& _path, BaseState _bs) :
        State(_accountStartNonce, _db, _bs) {
            dbUTXO = QtumCashState::openDB(_path + "/qtumcashDB", sha3(rlp("")), WithExisting::Trust);
	        stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

QtumCashState::QtumCashState() : dev::eth::State(dev::Invalid256, dev::OverlayDB(), dev::eth::BaseState::PreExisting) {
    dbUTXO = OverlayDB();
    stateUTXO = SecureTrieDB<Address, OverlayDB>(&dbUTXO);
}

ResultExecute QtumCashState::execute(EnvInfo const& _envInfo, SealEngineFace const& _sealEngine, QtumCashTransaction const& _t, Permanence _p, OnOpFunc const& _onOp){

    assert(_t.getVersion().toRaw() == VersionVM::GetEVMDefault().toRaw());

    addBalance(_t.sender(), _t.value() + (_t.gas() * _t.gasPrice()));
    newAddress = _t.isCreation() ? createQtumCashAddress(_t.getHashWith(), _t.getNVout()) : dev::Address();
    ...
    try{
        ...
        if (_p == Permanence::Reverted){
            ...
            qtumcash::commit(cacheUTXO, stateUTXO, m_cache);
            cacheUTXO.clear();
    ...
    if(voutLimit){
        ...
        return ResultExecute{ex, QtumCashTransactionReceipt(oldStateRoot, oldUTXORoot, gas, e.logs()), refund.vout.empty() ? CTransaction() : CTransaction(refund)};
    }else{
        return ResultExecute{res, QtumCashTransactionReceipt(rootHash(), rootHashUTXO(), startGasUsed + e.gasUsed(), e.logs()), tx ? *tx : CTransaction()};
    }
}

std::unordered_map<dev::Address, Vin> QtumCashState::vins() const // temp
{
...
void QtumCashState::transferBalance(dev::Address const& _from, dev::Address const& _to, dev::u256 const& _value) {
...
Vin const* QtumCashState::vin(dev::Address const& _a) const
{
    return const_cast<QtumCashState*>(this)->vin(_a);
}

Vin* QtumCashState::vin(dev::Address const& _addr)
{
...
// void QtumCashState::commit(CommitBehaviour _commitBehaviour)
// {
...
void QtumCashState::kill(dev::Address _addr)
{
...
void QtumCashState::addBalance(dev::Address const& _id, dev::u256 const& _amount)
{
...
void QtumCashState::deleteAccounts(std::set<dev::Address>& addrs){
...
void QtumCashState::updateUTXO(const std::unordered_map<dev::Address, Vin>& vins){
    for(auto& v : vins){
...
void QtumCashState::printfErrorLog(const dev::eth::TransactionException er){
    std::stringstream ss;
    ss << er;
    clog(dev::VerbosityWarning, "exec") << "VM exception:" << ss.str();
}

void QtumCashState::validateTransfersWithChangeLog(){
