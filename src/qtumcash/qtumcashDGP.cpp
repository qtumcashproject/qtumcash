#include <qtumcash/qtumcashDGP.h>
...
void QtumCashDGP::initDataSchedule(){
    dataSchedule = scheduleDataForBlockNumber(0);
}

bool QtumCashDGP::checkLimitSchedule(const std::vector<uint32_t>& defaultData, const std::vector<uint32_t>& checkData, int blockHeight){
...
dev::eth::EVMSchedule QtumCashDGP::getGasSchedule(int blockHeight){
    clear();
...
uint64_t QtumCashDGP::getUint64FromDGP(unsigned int blockHeight, const dev::Address& contract, std::vector<unsigned char> data){
    uint64_t value = 0;
...
uint32_t QtumCashDGP::getBlockSize(unsigned int blockHeight){
    clear();
...
uint64_t QtumCashDGP::getMinGasPrice(unsigned int blockHeight){
    clear();
...
uint64_t QtumCashDGP::getBlockGasLimit(unsigned int blockHeight){
    clear();
...
bool QtumCashDGP::initStorages(const dev::Address& addr, unsigned int blockHeight, std::vector<unsigned char> data){
    initStorageDGP(addr);
...
void QtumCashDGP::initStorageDGP(const dev::Address& addr){
    storageDGP = state->storage(addr);
}

void QtumCashDGP::initStorageTemplate(const dev::Address& addr){
    storageTemplate = state->storage(addr);
}

void QtumCashDGP::initDataTemplate(const dev::Address& addr, std::vector<unsigned char>& data){
    dataTemplate = CallContract(addr, data)[0].execRes.output;
}

void QtumCashDGP::createParamsInstance(){
...
dev::Address QtumCashDGP::getAddressForBlock(unsigned int blockHeight){
...
void QtumCashDGP::parseStorageScheduleContract(std::vector<uint32_t>& uint32Values){
...
void QtumCashDGP::parseDataScheduleContract(std::vector<uint32_t>& uint32Values){
...
void QtumCashDGP::parseStorageOneUint64(uint64_t& value){
...
void QtumCashDGP::parseDataOneUint64(uint64_t& value){
    if(dataTemplate.size() == 32){
        value = dev::toUint64(dev::u256(dev::h256(dataTemplate)));
    }
}

dev::eth::EVMSchedule QtumCashDGP::createEVMSchedule(const dev::eth::EVMSchedule &_schedule, int blockHeight){
