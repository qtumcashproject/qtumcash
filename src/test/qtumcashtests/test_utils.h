...
extern std::unique_ptr<QtumCashState> globalState;
...
inline void initState(){
    ...
    const std::string dirQtumCash = pathTemp.string();
    const dev::h256 hashDB(dev::sha3(dev::rlp("")));
    globalState = std::unique_ptr<QtumCashState>(new QtumCashState(dev::u256(0), QtumCashState::openDB(dirQtumCash, hashDB, dev::WithExisting::Trust), dirQtumCash + "/qtumcashDB", dev::eth::BaseState::Empty));
...
inline dev::Address createQtumCashAddress(dev::h256 hashTx, uint32_t voutNumber){
...
inline QtumCashTransaction createQtumCashTransaction(valtype data, dev::u256 value, dev::u256 gasLimit, dev::u256 gasPrice, dev::h256 hashTransaction, dev::Address recipient, int32_t nvout = 0){
    QtumCashTransaction txEth;
    if(recipient == dev::Address()){
        txEth = QtumCashTransaction(value, gasPrice, gasLimit, data, dev::u256(0));
    } else {
        txEth = QtumCashTransaction(value, gasPrice, gasLimit, recipient, data, dev::u256(0));
    }
...
inline std::pair<std::vector<ResultExecute>, ByteCodeExecResult> executeBC(std::vector<QtumCashTransaction> txs){
