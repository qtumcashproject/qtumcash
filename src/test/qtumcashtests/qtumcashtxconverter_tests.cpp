...
void checkResult(bool isCreation, std::vector<QtumCashTransaction> results, uint256 hash){
...
void runTest(bool isCreation, size_t n, CScript& script1, CScript script2 = CScript()){
    ...
    QtumCashTxConverter converter(transaction, NULL);
    ExtractQtumCashTX qtumcashTx;
    BOOST_CHECK(converter.extractionQtumCashTransactions(qtumcashTx));
    std::vector<QtumCashTransaction> result = qtumcashTx.first;
    if(script2 == CScript()){
...
BOOST_FIXTURE_TEST_SUITE(qtumcashtxconverter_tests, TestingSetup)
